	...
	maptype_t maptype, int outlumppos, int inlumppos)
{
	...
	for (i = 0; i < count; i++)
	{
		...
		out->flags = Mod_LoadSurfConvertFlags(LittleLong(in->surfaceFlags), maptype);
	...
	maptype_t maptype, int outlumppos, int inlumppos)
{
	...
	for (i = 0; i < count; i++)
	{
		...
		/* make unsigned long */
		out->firstleafface = LittleLong(in->firstLeafSurface) & 0xFFFFFFFF;
		out->numleaffaces = LittleLong(in->numLeafSurfaces) & 0xFFFFFFFF;
		out->firstleafbrush = LittleLong(in->firstLeafBrush) & 0xFFFFFFFF;
		out->numleafbrushes = LittleLong(in->numLeafBrushes) & 0xFFFFFFFF;
